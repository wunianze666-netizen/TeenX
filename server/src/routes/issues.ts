...
type CompanySearchService = {
  extract(
    ...
    scope: ReturnType<typeof genericIssueReadScopeForActor>,
...
export function issueRoutes(
  ...
  async function assertIssueReadAllowed(req: Request, res: Response, issue: Parameters<typeof decideIssueAccess>[1]) {
    if (isArenaSubmissionIssue(issue) && !canReadArenaIssue(req.actor, issue)) {
  ...
  function assertGenericIssueMutationAllowed(
    req: Request,
    res: Response,
    issue: Parameters<typeof decideIssueAccess>[1],
  ): boolean {
    const block = genericIssueMutationBlock(req.actor, issue);
    if (block === "not_found") {
      res.status(404).json({ error: "Issue not found" });
      return false;
    }
    if (block === "immutable") {
      res.status(409).json({ error: "Arena submission issues are immutable" });
      return false;
    }
    return true;
  }

  async function assertFeedbackTraceIssueReadAllowed(
    req: Request,
    res: Response,
    trace: { readonly issueId?: string | null },
  ): Promise<boolean> {
    if (!trace.issueId) return true;
    const issue = await svc.getById(trace.issueId);
    if (!issue || !actorCanAccessCompany(req, issue.companyId) || !canReadArenaIssue(req.actor, issue)) {
  ...
  async function assertAgentIssueCommentAllowed(
    ...
    if (!assertGenericIssueMutationAllowed(req, res, issue)) return false;
  ...
  async function filterIssuesForActor<T extends Parameters<typeof decideIssueAccess>[1]>(req: Request, rows: T[]) {
    ...
    return rows.filter((issue, index) => decisions[index]?.allowed && canReadArenaIssue(req.actor, issue));
  }

  async function filterIssueLinkedRowsForActor<T>(
    req: Request,
    rows: T[],
    issueIdFor: (row: T) => string | null,
  ): Promise<T[]> {
    const issueIds = [...new Set(rows.map(issueIdFor).filter((id): id is string => id !== null))];
    const loaded = await Promise.all(issueIds.map((id) => svc.getById(id)));
    const readable = await filterIssuesForActor(req, loaded.filter((issue) => issue !== null));
    const readableIds = new Set(readable.map((issue) => issue.id));
    return rows.filter((row) => {
      const issueId = issueIdFor(row);
      return issueId === null || readableIds.has(issueId);
    });
  }
  ...
  async function rejectAgentIssueThreadInteractionResolution(
    ...
    if (!assertGenericIssueMutationAllowed(req, res, issue)) return true;
    if (req.actor.type !== "agent") return false;
    if (
      req.actor.runId &&
  ...
  router.get("/companies/:companyId/search/extract", async (req, res) => {
    ...
    const result = await getSearchService().extract(
      companyId,
      parsedQuery.data,
      genericIssueReadScopeForActor(req.actor),
    );
    const results = await filterIssueLinkedRowsForActor(req, result.results, (row) => row.issueId);
  ...
  router.get("/companies/:companyId/search", async (req, res) => {
    ...
    const results = await filterIssueLinkedRowsForActor(
      req,
      result.results,
      (row) => row.issue?.id ?? row.artifact?.issueId ?? null,
    );
    res.json({ ...result, results });
  });
  ...
  router.get("/companies/:companyId/issues", async (req, res) => {
    ...
    const listFilters: IssueFilters = {
      ...
      genericIssueReadScope: genericIssueReadScopeForActor(req.actor),
      limit,
      offset,
    ...
    const coordinated = await coordinateIssueListGet({
      ...
      compute: async () => {
        const rawResult = (await svc.list(companyId, listFilters))
          .filter((issue) => canReadArenaIssue(req.actor, issue));
        ...
        return {
          kind: "full",
          body: result.map((issue) => projectArenaIssueForGenericRead({
            ...issue,
            successfulRunHandoff: handoffStates.get(issue.id) ?? null,
            activeRecoveryAction: recoveryActionByIssue.get(issue.id) ?? null,
          })),
  ...
  router.get("/issues/:id", async (req, res) => {
    ...
    const currentExecutionWorkspace = issue.executionWorkspaceId
      ...
      : null;
    const workProducts = (await workProductsSvc.listForIssue(issue.id)).map((product) =>
      isArenaSubmissionIssue(issue) ? projectArenaWorkProductForGenericRead(product) : product);
    const publicIssue = projectArenaIssueForGenericRead(issue);
    res.json({
      ...publicIssue,
      ...inboxArchiveFields,
      goalId: goal?.id ?? issue.goalId,
  ...
  router.get("/issues/:id/work-products", async (req, res) => {
    ...
    res.json(isArenaSubmissionIssue(issue) ? workProducts.map(projectArenaWorkProductForGenericRead) : workProducts);
  });
    ...
    const candidateIssues = requestedIssueIds.length > 0
      ? await db
        .select({
          ...
          responsibleUserId: issueRows.responsibleUserId,
        })
        .from(issueRows)
  ...
  router.patch("/work-products/:id", validate(updateIssueWorkProductSchema), async (req, res) => {
    ...
    if (issue.originKind === "advx_arena_submission" && existing.metadata?.arenaScore) {
      res.status(409).json({ error: "Arena scorecards are immutable" });
      return;
    }
    if (!(await assertAgentIssueMutationAllowed(req, res, issue))) return;
    if (!(await assertDeliverableMutationAllowedByRunContext(req, res, issue))) return;
  ...
  router.patch("/issues/:id", validate(updateIssueRouteSchema), async (req, res) => {
    ...
    const existing = await getAccessibleResource(req, res, svc.getById(id), "Issue not found");
    if (!existing) return;
    if (existing.originKind === "advx_arena_submission") {
      res.status(409).json({ error: "Arena submissions are immutable" });
      return;
    }
    assertNoAgentHostWorkspaceCommandMutation(req, collectIssueWorkspaceCommandPaths(req.body));
    if (!(await assertAgentIssueMutationAllowed(req, res, existing))) return;
  ...
  router.get("/issues/:id/interactions", async (req, res) => {
    ...
    if (!isArenaSubmissionIssue(issue)) {
  ...
  router.post("/issues/:id/interactions", validate(createIssueThreadInteractionSchema), async (req, res) => {
    ...
    const issue = await getAccessibleResource(req, res, svc.getById(id), "Issue not found");
    if (!issue) return;
    if (!assertGenericIssueMutationAllowed(req, res, issue)) return;
    if (req.actor.type === "agent") {
      if (!(await assertAgentIssueMutationAllowed(req, res, issue))) return;
      if (await assertLowTrustControlPlaneDenied(req, res, issue.companyId, issue)) return;
    } else {
  ...
  router.get("/feedback-traces/:traceId", async (req, res) => {
    ...
    if (!(await assertFeedbackTraceIssueReadAllowed(req, res, trace))) return;
    res.json(trace);
  });
  ...
  router.get("/feedback-traces/:traceId/bundle", async (req, res) => {
    ...
    if (!(await assertFeedbackTraceIssueReadAllowed(req, res, bundle))) return;
    res.json(bundle);
  });
  ...
  router.get("/issues/:id/attachments", async (req, res) => {
    ...
    if (!issue) return;
    if (issue.originKind === "advx_arena_submission") {
      res.status(404).json({ error: "Issue not found" });
      return;
    }
    if (!(await assertIssueReadAllowed(req, res, issue))) return;
