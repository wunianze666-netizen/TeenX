...
export function issueTreeControlRoutes(db: Db) {
  ...
  function assertTreeReadAllowed(
    req: Request,
    res: Response,
    root: { readonly id: string; readonly originKind?: string | null; readonly responsibleUserId?: string | null },
  ): boolean {
    if (canReadArenaIssue(req.actor, root)) return true;
    res.status(404).json({ error: "Root issue not found" });
    return false;
  }

  function assertTreeMutationAllowed(
    req: Request,
    res: Response,
    root: { readonly id: string; readonly originKind?: string | null; readonly responsibleUserId?: string | null },
  ): boolean {
    const block = genericIssueMutationBlock(req.actor, root);
    if (block === "not_found") {
      res.status(404).json({ error: "Root issue not found" });
      return false;
    }
  ...
  router.post("/issues/:id/tree-control/preview", validate(previewIssueTreeControlSchema), async (req, res) => {
    ...
    if (!root) return;
    if (!assertTreeMutationAllowed(req, res, root)) return;
  ...
  router.get("/issues/:id/tree-control/state", async (req, res) => {
    ...
    if (!issue) return;
    if (!assertTreeReadAllowed(req, res, issue)) return;
  ...
  router.get("/issues/:id/tree-holds", async (req, res) => {
    ...
    const root = await getAccessibleResource(req, res, resolveRootIssue(req), "Root issue not found");
    if (!root) return;
    if (!assertTreeReadAllowed(req, res, root)) return;
