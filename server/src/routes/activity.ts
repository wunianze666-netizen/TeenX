...
export function activityRoutes(db: Db) {
  ...
  async function assertIssueReadAllowed(req: Parameters<typeof assertCompanyAccess>[0], res: any, issue: {
    ...
    if (!canReadArenaIssue(req.actor, issue)) {
  ...
  router.post("/companies/:companyId/activity", validate(createActivitySchema), async (req, res) => {
    ...
    const companyId = req.params.companyId as string;
    assertCompanyAccess(req, companyId);
    if (req.body.entityType === "issue") {
      const issue = await issueSvc.getById(req.body.entityId);
  ...
  router.get("/issues/:id/activity", async (req, res) => {
    ...
    const result = await svc.forIssue(
      issue.companyId,
      issue.id,
  ...
  router.get("/issues/:id/runs", async (req, res) => {
    ...
    if (!issue) return;
    if (!(await assertIssueReadAllowed(req, res, issue))) return;
    const result = await svc.runsForIssue(
      issue.companyId,
      issue.id,
      genericIssueReadScopeForActor(req.actor),
    );
  ...
  router.get("/heartbeat-runs/:runId/issues", async (req, res) => {
    ...
    const result = await svc.issuesForRun(runId, genericIssueReadScopeForActor(req.actor));
    res.json(result);
  });
