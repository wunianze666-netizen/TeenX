...
import { advxArenaRoutes } from "./routes/advx-arena.js";
...
import { createTeenxProfileStore } from "./services/teenx-profile-store.js";
...
export async function createApp(
  ...
  opts: {
    uiMode: UiMode;
    readonly advxServerProfile?: AdvxServerProfile;
    serverPort: number;
    ...
    teenxChildConfig: TeenxChildConfig;
  },
) {
  const advxServerProfile = opts.advxServerProfile ?? "real";
  const teenxChildConfig = opts.teenxChildConfig;
  const teenxProfileConfig = readTeenxProfileConfig(process.env, teenxChildConfig, {
    deploymentMode: opts.deploymentMode,
    deploymentExposure: opts.deploymentExposure,
  });
  const app = express();
  app.locals.paperclipDb = db;
  app.locals.advxServerProfile = advxServerProfile;
  ...
  app.use(teenxChildApiBoundary(teenxChildConfig));
  app.locals[TEENX_CHILD_API_BOUNDARY_INSTALLED] = true;
  assertTeenxChildBoundaryInstalled(
    teenxChildConfig,
    app.locals[TEENX_CHILD_API_BOUNDARY_INSTALLED] === true,
  );
  ...
  const arenaRuntime = await createProfiledArenaRuntime(db, opts.storageService, advxServerProfile);
  app.locals.advxArenaRuntime = arenaRuntime;
  const arenaRecovery = arenaRuntime.recover().then(() => true, (err) => {
    logger.error({ err }, "Failed to recover ADVX Arena runs on startup");
    return false;
  });
  ...
  api.use(
    ...
    healthRoutes(db, {
      ...
      arenaHealth: arenaRuntime.health,
    }),
  );
  api.use(openApiRoutes());
  api.use("/companies", companyRoutes(db, opts.storageService));
  const profileStore = createTeenxProfileStore(db);
  api.use("/advx", advxSessionRoutes());
  api.use("/advx", advxProfileSsoRoutes({ config: teenxProfileConfig, store: profileStore }));
  if (teenxChildConfig.enabled && teenxProfileConfig.enabled) {
    api.use("/advx", advxProfileRoutes({
      config: teenxProfileConfig,
      bridge: createTeenxBridgeClient(teenxProfileConfig),
      directory: createTeenxPublicDirectory({
        secret: teenxProfileConfig.publicIdSecret,
        scanCap: teenxProfileConfig.publicIdScanCap,
        cacheTtlMs: teenxProfileConfig.publicIdCacheTtlMs,
        loadEligibleCaptains: profileStore.loadEligibleCaptains,
      }),
      store: profileStore,
    }));
  }
  api.use(
    "/advx/demo",
    advxDemoRoutes(advxServerProfile, createAdvxDemoService(db, arenaRuntime, advxServerProfile)),
  );
  api.use("/advx/arena", advxArenaRoutes(db, opts.storageService, arenaRuntime, arenaRecovery));
  api.use("/advx", advxRoutes(db, { profile: advxServerProfile }));
  ...
  const toolDispatcherStartup = toolDispatcher.initialize().catch((err) => {
  ...
  const pluginLoaderStartup = ensureBundledPlugins(
  ...
  const shutdownAppServices = async () => {
    ...
    await arenaRuntime.shutdown();
    disableFeedbackExportFlushes();
    await Promise.all([toolDispatcherStartup, pluginLoaderStartup]);
    devWatcher?.close();
