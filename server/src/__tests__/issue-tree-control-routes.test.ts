...
describe("issue tree control routes", () => {
  ...
  it("hides foreign-captain Arena tree state before querying tree services", async () => {
    mockIssueService.getById.mockResolvedValue({
      id: "11111111-1111-4111-8111-111111111111",
      companyId: "company-2",
    ...
    const res = await request(app)
      .get("/api/issues/11111111-1111-4111-8111-111111111111/tree-control/state");

    expect(res.status).toBe(404);
    expect(res.body).toEqual({ error: "Root issue not found" });
    expect(mockTreeControlService.getActivePauseHoldGate).not.toHaveBeenCalled();
  });

  it.each([
    ["preview", "/api/issues/11111111-1111-4111-8111-111111111111/tree-control/preview", { mode: "pause" }],
    ["create hold", "/api/issues/11111111-1111-4111-8111-111111111111/tree-holds", { mode: "pause", reason: "blocked" }],
    ["release hold", "/api/issues/11111111-1111-4111-8111-111111111111/tree-holds/33333333-3333-4333-8333-333333333333/release", { reason: "blocked" }],
  ])("blocks Arena tree %s before all side effects", async (_name, path, body) => {
    mockIssueService.getById.mockResolvedValue({
      id: "11111111-1111-4111-8111-111111111111",
      companyId: "company-2",
      originKind: "advx_arena_submission",
