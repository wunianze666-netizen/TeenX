...
describe("agent issue mutation checkout ownership", () => {
  ...
  it("uses the company-scope fast path on the issue list route", async () => {
    ...
    expect(mockIssueService.list).toHaveBeenCalledWith(companyId, expect.objectContaining({
      genericIssueReadScope: { captainId: "board-user" },
    }));
  });

  it("passes the captain scope into issue counts before aggregation", async () => {
    const res = await request(await createApp(boardActor()))
      .get(`/api/companies/${companyId}/issues/count?attention=blocked`);

    expect(res.status, JSON.stringify(res.body)).toBe(200);
    expect(res.body).toEqual({ count: 1 });
    expect(mockIssueService.count).toHaveBeenCalledWith(companyId, expect.objectContaining({
      genericIssueReadScope: { captainId: "board-user" },
    }));
  });
  ...
  it("rejects accepted decomposition reads outside the issue boundary", async () => {
  ...
  it("blocks Arena accepted decomposition before creating child issues", async () => {
    mockIssueService.getById.mockResolvedValue(makeIssue({
