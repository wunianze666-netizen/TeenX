...
describeEmbeddedPostgres("issueService.list participantAgentId", () => {
  ...
  it("excludes foreign-captain Arena issues before list pagination and counts", async () => {
    ...
    const filters = { genericIssueReadScope: { captainId: "captain-b" } } as const;

    const result = await svc.list(companyId, { ...filters, limit: 1 });
    const count = await svc.count(companyId, filters);

    expect(result.map((issue) => issue.id)).toEqual([ordinaryId]);
    expect(count).toBe(1);
  });
