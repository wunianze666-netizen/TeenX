...
describeEmbeddedPostgres("activity service", () => {
  ...
  it("excludes foreign-captain Arena activity before applying the limit", async () => {
    const companyId = randomUUID();
    const ordinaryNewestId = randomUUID();
    const foreignArenaId = randomUUID();
    const ordinaryOldestId = randomUUID();
    await db.insert(companies).values({
      id: companyId,
      name: "Paperclip",
      issuePrefix: `T${companyId.replace(/-/g, "").slice(0, 6).toUpperCase()}`,
    ...
    await db.insert(issues).values([
      { id: ordinaryNewestId, companyId, title: "Newest", status: "todo", priority: "medium" },
      {
        id: foreignArenaId,
        companyId,
        title: "Foreign Arena",
      ...
      { id: ordinaryOldestId, companyId, title: "Oldest", status: "todo", priority: "medium" },
    ]);
    await db.insert(activityLog).values([
      { companyId, actorType: "system", actorId: "system", action: "ordinary.newest", entityType: "issue", entityId: ordinaryNewestId, createdAt: new Date("2026-04-21T12:00:00.000Z") },
      { companyId, actorType: "system", actorId: "system", action: "arena.foreign", entityType: "issue", entityId: foreignArenaId, createdAt: new Date("2026-04-21T11:00:00.000Z") },
      { companyId, actorType: "system", actorId: "system", action: "ordinary.oldest", entityType: "issue", entityId: ordinaryOldestId, createdAt: new Date("2026-04-21T10:00:00.000Z") },
    ]);

    const result = await activityService(db).list({
      companyId,
      limit: 2,
      genericIssueReadScope: { captainId: "captain-b" },
    });

    expect(result.map((event) => event.action)).toEqual(["ordinary.newest", "ordinary.oldest"]);
  });
