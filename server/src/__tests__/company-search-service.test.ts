...
describeEmbeddedPostgres("companySearchService", () => {
  ...
  it("excludes foreign-captain Arena matches before pagination and aggregates", async () => {
    const companyId = await createCompany();
    const base = new Date("2026-01-01T00:00:00.000Z").getTime();
    const ordinaryId = await createIssue(companyId, {
      identifier: "ORD-1",
      title: "Needle ordinary",
      updatedAt: new Date(base),
    });
    await createIssue(companyId, {
      identifier: "ARENA-1",
      title: "Needle foreign Arena",
    ...
    const result = await svc.search(
      companyId,
      companySearchQuerySchema.parse({ q: "needle", scope: "issues", limit: "1" }),
      { captainId: "captain-b" },
    );

    expect(result.results.map((item) => item.id)).toEqual([ordinaryId]);
    expect(result.countsByType.issue).toBe(1);
    expect(result.filterOptionCounts.status.todo).toBe(1);
    expect(result.hasMore).toBe(false);
  });

  it("keeps the owning captain's Arena issue-backed artifacts searchable", async () => {
    const companyId = await createCompany();
    const agentId = await createAgent(companyId, { name: "Artifact Writer" });
    const issueId = await createIssue(companyId, {
      identifier: "ARENA-1",
      title: "Arena artifact holder",
      originKind: "advx_arena_submission",
      responsibleUserId: "captain-a",
    });
    const documentId = randomUUID();
    await db.insert(documents).values({
      id: documentId,
      companyId,
      title: "Arena searchable artifact",
      latestBody: "needle artifact",
      format: "markdown",
      createdByAgentId: agentId,
    });
    await db.insert(issueDocuments).values({ companyId, issueId, documentId, key: "artifact" });

    const ownerResult = await svc.search(
      companyId,
      companySearchQuerySchema.parse({ q: "needle", scope: "artifacts" }),
      { captainId: "captain-a" },
    );
    const foreignResult = await svc.search(
      companyId,
      companySearchQuerySchema.parse({ q: "needle", scope: "artifacts" }),
      { captainId: "captain-b" },
    );

    expect(ownerResult.results.map((item) => item.artifact?.issueId)).toEqual([issueId]);
    expect(ownerResult.countsByType.artifact).toBe(1);
    expect(foreignResult.results).toEqual([]);
    expect(foreignResult.countsByType.artifact).toBe(0);
  });
