...
async function createApp(actor: Express.Request["actor"], bulkIssue: Record<string, unknown> = makeIssue()) {
  ...
  const routeDb = {
    select: vi.fn((selection: Record<string, unknown>) => ({
      from: vi.fn(() => ({
        where: vi.fn(async () => [Object.fromEntries(
          Object.keys(selection).map((key) => [key, bulkIssue[key]]),
        )]),
      })),
    })),
  };
...
describe("external object routes", () => {
  ...
  it("filters foreign-captain Arena issues from bulk summaries before fetching summaries", async () => {
    const app = await createApp(boardActor(), makeIssue({
