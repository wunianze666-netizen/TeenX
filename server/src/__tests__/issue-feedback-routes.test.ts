...
describe("issue feedback trace routes", () => {
  ...
  it.each([
    ["votes", "/api/issues/issue-1/feedback-votes", "listIssueVotesForUser"],
    ["traces", "/api/issues/issue-1/feedback-traces", "listFeedbackTraces"],
  ])("hides foreign-captain Arena feedback %s before service reads", async (_name, path, serviceMethod) => {
    mockIssueService.getById.mockResolvedValue({
      id: "issue-1",
      companyId: "company-1",
      projectId: null,
    ...
    expect(mockFeedbackService[serviceMethod as "listIssueVotesForUser" | "listFeedbackTraces"]).not.toHaveBeenCalled();
  });

  it("blocks Arena feedback votes before persistence and export side effects", async () => {
    mockIssueService.getById.mockResolvedValue({
      id: "issue-1",
      companyId: "company-1",
      identifier: "ARENA-1",
  ...
  it.each([
    ["trace", "/api/feedback-traces/trace-1", "getFeedbackTraceById"],
    ["bundle", "/api/feedback-traces/trace-1/bundle", "getFeedbackTraceBundle"],
  ])("hides a foreign-captain Arena feedback %s by linked issue", async (_name, path, serviceMethod) => {
    mockFeedbackService[serviceMethod as "getFeedbackTraceById" | "getFeedbackTraceBundle"].mockResolvedValue({
      id: "trace-1",
      companyId: "company-1",
      issueId: "issue-1",
      files: [],
    ...
    expect(res.status).toBe(404);
    expect(res.body).toEqual({ error: "Feedback trace not found" });
  });
});
