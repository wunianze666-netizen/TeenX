...
describeEmbeddedPostgres("companySearchExtractService", () => {
  ...
  it("excludes foreign-captain Arena matches before extract pagination", async () => {
    const companyId = await createCompany();
    const base = new Date("2026-01-01T00:00:00.000Z").getTime();
    const ordinaryId = await createIssue(companyId, {
      identifier: "EXT-ORDINARY",
      description: "needle ordinary",
      updatedAt: new Date(base),
    });
    await createIssue(companyId, {
      identifier: "EXT-ARENA",
      description: "needle foreign Arena",
    ...
    const result = await svc.extract(
      companyId,
      companySearchExtractQuerySchema.parse({ contains: "needle", limit: "1" }),
      { captainId: "captain-b" },
    );

    expect(result.results.map((row) => row.issueId)).toEqual([ordinaryId]);
