...
export function activityService(db: Db) {
  ...
  return {
    ...
    forIssue: (
      companyId: string,
      issueId: string,
      genericReadScope: GenericIssueReadScope = { captainId: null },
    ) =>
      db
        .select({ activityLog })
        .from(activityLog)
        ...
        .where(
          and(
            ...
            visibleIssueCondition(),
            genericIssueReadCondition(genericReadScope),
          ),
        )
        .orderBy(desc(activityLog.createdAt))
    ...
    runsForIssue: async (
      ...
      const boundaryIssue = await db
        ...
        .then((rows) => rows[0] ?? null);
      if (boundaryIssue && !canReadArenaIssueInScope(genericReadScope, boundaryIssue)) return [];
