...
import {
  genericIssueReadCondition,
  type GenericIssueReadScope,
} from "./advx-arena/generic-issue-boundary.js";
...
export interface IssueFilters {
  ...
  genericIssueReadScope?: GenericIssueReadScope;
  q?: string;
  limit?: number;
  offset?: number;
  sortField?: "updated";
...
async function blockedInboxIssueConditions(
  ...
  const conditions = [
    eq(issues.companyId, companyId),
    visibleIssueCondition(),
    genericIssueReadCondition(filters?.genericIssueReadScope ?? { captainId: null }),
  ...
  return {
    ...
    countUnreadTouchedByUser: async (
      ...
      const conditions = [
        eq(issues.companyId, companyId),
        visibleIssueCondition(),
        genericIssueReadCondition({ captainId: userId }),
    ...
    update: async (
      ...
      if (existing.originKind === "advx_arena_submission" && issueData.executionState !== undefined) {
        const existingState = existing.executionState && typeof existing.executionState === "object"
          ? existing.executionState
          : {};
        const requestedState = issueData.executionState && typeof issueData.executionState === "object"
          ? issueData.executionState
          : {};
        patch.executionState = { ...requestedState, arena: existingState.arena };
      }
